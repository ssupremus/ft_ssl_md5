...
void sha384512_update(t_ssl *ssl, const unsigned char *input, size_t length)
{
	...
	if (left && length >= ssl->len_bit)
	{
		...
		sha384512_process(ssl, ssl->buffer);
		input += ssl->len_bit;
		length -= ssl->len_bit;
		left = 0;
	}
...
void sha384512_finish(t_ssl *ssl)
{
	...
	sha384512_update(ssl, ssl->padded_message, padn);
	sha384512_update(ssl, msglen, 16);
}