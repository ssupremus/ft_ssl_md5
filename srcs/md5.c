...
int		md5(t_ssl *ssl, int length, char **args)
{
	...
	size_t len = ft_strlen(line) + 1;
	while (len % 64 != 56)
		len++;
	void *msg;

	if (!(msg = malloc(len + 64)))
		return (-1);
	ft_bzero(msg, len + 64);
	ft_strcpy((char *)msg, (const char *)line);
	*(uint32_t*)(msg + ft_strlen(line)) = 0x80;
	*(uint32_t*)(msg + len) = (uint32_t)(8 * ft_strlen(line));
	size_t offset = 0;
